...
Date   : 8/21/18
Purpose:
    This is a program that multiplies two matrices. This version is 
    (hopefully) faster.
Debug  : 
Notes  : 
    1. How to Run:
    ...
    12. Fails with nvcc --gpu-architecture=sm_72 src/matrix_multiply_improved.cu with 'no kernel image'
        error.
...
/********************************************************
    ...
    RETURN:
    DEBUG:
        1. This returns shit even though this is the same function as
           in matrix_multiply.cu.  I don't understand it.  
    NOTES: 
    ...
    for(ai=startIdx; ai<dimA[0]; ai+=stride){
        ...
        for(bj=0; bj<dimB[1]; bj++){
            ...
            for(j=0; j<dimA[1]; j++){
                ...
                //printf("%.1f (A[%i,%i]) * %.1f (B[%i,%i]) = %.1f\n",
                //       A[d_map_idx(ai, j, dimA[1])], ai, j, B[d_map_idx(j, bj, dimB[1])],
                //       j, bj, A[d_map_idx(ai, j, dimA[1])] * B[d_map_idx(j, bj, dimB[1])]);
...
int main(int argc, char *argv[])
{
    ...
    int * dimA = (int *)malloc(2 * sizeof(int)); 
    int * dimB = (int *)malloc(2 * sizeof(int)); 
    int * dimAB = (int *)malloc(2 * sizeof(int)); // Initialize to some value
    float *A = NULL;
    float *B = NULL;
    float *AB = NULL;
    float *answer = NULL;
    ...
    dimAB[0] = dimA[0];
    dimAB[1] = dimB[1];
    AB = (float *)malloc(dimAB[0] * dimAB[1] * sizeof(float));
    ...
    int * d_dimAB;
    // allocate
    gpuErrChk(cudaMalloc(&d_dimA,  2 * sizeof(int)));
    gpuErrChk(cudaMalloc(&d_dimB,  2 * sizeof(int)));
    gpuErrChk(cudaMalloc(&d_dimAB, 2 * sizeof(int)));
    //copy
    gpuErrChk(cudaMemcpy(d_dimA, dimA, 2 * sizeof(int), cudaMemcpyHostToDevice));
    gpuErrChk(cudaMemcpy(d_dimB, dimB, 2 * sizeof(int), cudaMemcpyHostToDevice));
    gpuErrChk(cudaMemcpy(d_dimAB, dimAB, 2 * sizeof(int), cudaMemcpyHostToDevice));
    // allocate - use host code b/c can't access device mem here.
    gpuErrChk(cudaMalloc(&d_A,  dimA[0]  * dimA[1]  * sizeof(float)));
    gpuErrChk(cudaMalloc(&d_B,  dimB[0]  * dimB[1]  * sizeof(float)));
    gpuErrChk(cudaMalloc(&d_AB, dimAB[0] * dimAB[1] * sizeof(float)));
    //copy
    gpuErrChk(cudaMemcpy(d_A,  A, dimA[0] * dimA[1] * sizeof(float), cudaMemcpyHostToDevice));
    gpuErrChk(cudaMemcpy(d_B,  B, dimB[0] * dimB[1] * sizeof(float), cudaMemcpyHostToDevice));

    //            <<<gridDim.x (# blocks), blockDim.x (# threads per block) >>>

    time_t start = time(NULL);
    matrix_multiply<<<32,32>>> (d_A, d_B, d_dimA, d_dimB, d_AB, d_dimAB);  // Fails b/c maxThreadsPerBlock=1024
    gpuErrChk(cudaPeekAtLastError());
    gpuErrChk(cudaDeviceSynchronize());
    gpuErrChk(cudaMemcpy(AB,  d_AB, dimAB[0] * dimAB[1] * sizeof(float), cudaMemcpyDeviceToHost));
